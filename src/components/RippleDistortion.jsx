...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    const overFolder = event =>
      Boolean(event.target?.closest?.('.folder-stage, .page-swap, .page-sheet, .about-story'));

    const onMove = event => {
      const cfg = configRef.current;
      if (!cfg.enabled || reduceMotion || cfg.trigger === 'click') return;
      if (overFolder(event)) return;
    ...
    const updateFolderShadow = () => {
      const stage = mount.parentElement?.querySelector('.folder-stage');
      if (!stage) return;
      const mountRect = mount.getBoundingClientRect();
      const fr = stage.getBoundingClientRect();
      const cx = fr.left + fr.width / 2;
      const cy = fr.top + fr.height * 0.72;
      let tx = 0;
      let ty = 0;
      let skew = 0;
      let spread = 1;
      ...
      for (let i = 0; i < MAX_WAVES; i += 1) {
        const wave = waves[i];
        if (wave.opacity <= 0.01) continue;
        const wx = mountRect.left + wave.x;
        const wy = mountRect.top + (height - wave.y);
        const dx = cx - wx;
        const dy = cy - wy;
        const dist = Math.sqrt(dx * dx + dy * dy) || 1;
        const reach = 240 + wave.scale * wave.size * 0.12;
        if (dist > reach) continue;
        const w = wave.opacity * (1 - dist / reach);
        tx += (dx / dist) * w * 22;
        ty += (dy / dist) * w * 14;
        skew += ((wx - cx) / dist) * w * 7;
        spread += w * 0.18;
      ...
      shadowOx += (tx - shadowOx) * 0.2;
      shadowOy += (ty - shadowOy) * 0.2;
      shadowSkew += (skew - shadowSkew) * 0.2;
      shadowSpread += (spread - shadowSpread) * 0.2;

      stage.style.setProperty('--ripple-sx', `${shadowOx.toFixed(2)}px`);
      stage.style.setProperty('--ripple-sy', `${shadowOy.toFixed(2)}px`);
      stage.style.setProperty('--ripple-skew', `${shadowSkew.toFixed(2)}deg`);
      stage.style.setProperty('--ripple-spread', shadowSpread.toFixed(3));
    };

    const grabBackground = () => {
      const bg = mount.parentElement?.querySelector('.molten-metal-container canvas');
      if (!bg || bg.width < 2 || bg.height < 2) return;
      imageTexture.image = bg;
