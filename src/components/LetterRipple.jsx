...
void main() {
  ...
  vec2 sampleUV = clamp(base + push, 0.0, 1.0);

  vec3 color;
  if (uDispersion > 0.001) {
    float split = uDispersion * 0.25;
    color.r = texture2D(uTexture, clamp(base + push * (1.0 + split), 0.0, 1.0)).r;
    color.g = texture2D(uTexture, sampleUV).g;
    color.b = texture2D(uTexture, clamp(base + push * (1.0 - split), 0.0, 1.0)).b;
  } else {
...
const LetterRipple = ({
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      bake.width = Math.max(1, image.naturalWidth);
      bake.height = Math.max(1, image.naturalHeight);
      const ctx = bake.getContext('2d', { alpha: false });
      ctx.drawImage(image, 0, 0);
      imageTexture.image = bake;
      imageTexture.needsUpdate = true;
      compositeUniforms.uTextureSize.value = [bake.width, bake.height];
    };
    image.src = src;
  ...
  return <div ref={mountRef} className={`letter-ripple ${className}`.trim()} style={style} />;
};

export default LetterRipple;
