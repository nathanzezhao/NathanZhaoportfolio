...
void main() {
  ...
  vec3 normal = normalize(vec3(-ex * 70.0, -ey * 70.0, 0.45));
  vec3 view = vec3(0.0, 0.0, 1.0);
  vec3 light = normalize(vec3(-0.42, 0.82, 0.38));
  vec3 fill = normalize(vec3(0.75, -0.2, 0.4));
  vec3 R = reflect(-view, normal);
  float ndv = max(dot(normal, view), 0.0);
  float fresnel = pow(1.0 - ndv, 1.7);

  float sky = clamp(R.y * 0.5 + 0.5, 0.0, 1.0);
  vec3 env = mix(vec3(0.04, 0.045, 0.055), vec3(0.55, 0.58, 0.62), pow(sky, 0.7));
  env += vec3(0.85, 0.9, 1.0) * pow(max(R.x * 0.3 + R.y * 0.85, 0.0), 8.0) * 0.55;

  float spec = pow(max(dot(normal, light), 0.0), 28.0);
  float hot = pow(max(dot(normal, light), 0.0), 90.0);
  float spec2 = pow(max(dot(normal, fill), 0.0), 70.0);

  float glass = smoothstep(0.001, 0.04, amount);
  vec3 chrome = env;
  chrome += vec3(1.0) * spec * 1.1;
  chrome += vec3(1.0) * hot * 1.6;
  chrome += vec3(0.9, 0.93, 1.0) * spec2 * 0.55;
  chrome += vec3(0.75, 0.82, 0.95) * fresnel * 0.7;
  chrome *= mix(1.0, 0.72, (1.0 - fresnel) * glass);
...
const MercuryRipple = ({
  ...
  useEffect(() => {
    ...
    const overWheel = event => Boolean(event.target?.closest?.('.option-wheel, .nav-slot'));
  ...
  return <div ref={mountRef} className={`mercury-ripple ${className}`.trim()} style={style} />;
};

export default MercuryRipple;
