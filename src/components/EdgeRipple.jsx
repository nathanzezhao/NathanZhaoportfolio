...
vec2 keepInside(vec2 base, vec2 push) {
  vec2 t = vec2(1.0);
  if (push.x > 0.0) t.x = (1.0 - base.x) / max(push.x, 1.0e-6);
  else if (push.x < 0.0) t.x = base.x / max(-push.x, 1.0e-6);
  if (push.y > 0.0) t.y = (1.0 - base.y) / max(push.y, 1.0e-6);
  else if (push.y < 0.0) t.y = base.y / max(-push.y, 1.0e-6);
  return base + push * clamp(min(t.x, t.y), 0.0, 1.0);
}

void main() {
  vec2 base = fitUV(vUv);
  vec2 origin = clamp(base, vec2(0.0), vec2(1.0));
  bool letterbox = base.x < 0.0 || base.x > 1.0 || base.y < 0.0 || base.y > 1.0;

  float amount = textureLod(uDisplacement, vUv, 0.0).r;
  if (letterbox && amount < 0.002) {
  ...
  vec2 sampleUV = keepInside(origin, push);

  vec3 color;
  if (uDispersion > 0.001) {
    float split = uDispersion * 0.25;
    color.r = sampleImage(keepInside(origin, push * (1.0 + split))).r;
    color.g = sampleImage(sampleUV).g;
    color.b = sampleImage(keepInside(origin, push * (1.0 - split))).b;
  } else {
...
const EdgeRipple = ({
  ...
  return <div ref={mountRef} className={`edge-ripple ${className}`.trim()} style={style} />;
};

export default EdgeRipple;
